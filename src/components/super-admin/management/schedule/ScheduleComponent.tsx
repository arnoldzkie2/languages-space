...
const ScheduleComponent: React.FC<Props> = (schedule) => {
    ...
    return (
        <div onClick={() => status === 'reserved' && booking![0].id ? viewBooking(booking![0].id) :
            bindSchedule(scheduleID)} className={`p-2 mx-2 my-0.5 w-full border hover:border-blue-600 ${status === 'reserved' ?
                'bg-blue-600 text-white' : 'bg-slate-200 text-gray-600'} cursor-pointer rounded-md flex items-center gap-2`}>
