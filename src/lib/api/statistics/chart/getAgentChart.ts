...
export const getWeeklyAgentCountChart = async () => {
    ...
    const currentWeek = await prisma.agent.findMany({
        where: {
            created_at: {
    ...
    const previousWeek = await prisma.agent.findMany({
        where: {
            created_at: {
    ...
    // Initialize objects to hold agent counts for each day of the week
    const currentWeekCounts: Record<string, number> = {};
    const previousWeekCounts: Record<string, number> = {};

    // Count the number of agent for each day of the current week
    currentWeek.forEach(agent => {
        const day = format(agent.created_at, 'eeee'); // Get the day name (e.g., Monday)
        currentWeekCounts[day] = (currentWeekCounts[day] || 0) + 1;
    });

    // Count the number of agent for each day of the previous week
    previousWeek.forEach(agent => {
        const day = format(agent.created_at, 'eeee'); // Get the day name (e.g., Monday)
        previousWeekCounts[day] = (previousWeekCounts[day] || 0) + 1;
    });

    // Prepare data structure with total agent counts for each day of the current and previous week
...
export const getMonthlyAgentCountChart = async () => {
    ...
    const currentMonth = await prisma.agent.findMany({
        where: {
            created_at: {
                gte: startDate,
                lte: endDate
    ...
    const previousMonth = await prisma.agent.findMany({
        where: {
            created_at: {
    ...
    // Count the number of agent for each day of the current month
    const currentMonthCount: Record<number, number> = {};
    currentMonth.forEach(agent => {
        const dayOfMonth = agent.created_at.getDate();
        currentMonthCount[dayOfMonth] = (currentMonthCount[dayOfMonth] || 0) + 1;
    });

    // Count the number of agent for each day of the previous month
    const previousMonthCount: Record<number, number> = {};
    previousMonth.forEach(agent => {
        const dayOfMonth = agent.created_at.getDate();
        previousMonthCount[dayOfMonth] = (previousMonthCount[dayOfMonth] || 0) + 1;
    });

    // Prepare data structure with total agent counts for each day of the current and previous month
...
export const getYearlyAgentCountChart = async () => {
    ...
    // Fetch agent within the current year
    const currentYear = await prisma.agent.findMany({
        where: {
            created_at: {
                gte: startDate,
                lte: endDate
    ...
    // Fetch agent within the previous year
    const previousYear = await prisma.agent.findMany({
        where: {
            created_at: {
    ...
    // Count the number of agent for each month of the current year
    ...
    // Count the number of agent for each month of the previous year
