...
const AdminBookingRequest: React.FC<Props> = ({ filteredTable }) => {
                        ...
                        <tr className="bg-white border hover:bg-slate-50" key={booking.id}>
                            ...
                            <td className='py-3 relative px-2'>
                                ...
                                <ul className={`${operation && selectedID === booking.id ? 'block' : 'hidden'} absolute bg-white p-3 gap-1 z-10 w-24 shadow-lg border flex flex-col text-gray-600`}>
                                    {booking.status !== 'canceled' && permissions?.cancel_booking_request && <button disabled={isLoading} className='flex mb-1 justify-between items-center cursor-dpointer hover:text-red-600' onClick={(e: any) => cancelBookingRequest(e, booking.id)}>
                                    ...
                                    {permissions?.delete_booking_request && <li className='flex mb-1 justify-between items-center cursor-pointer hover:text-red-600' onClick={() => openDeleteBookingReqeustWarningMOdal(booking)}>{tt('delete')} <FontAwesomeIcon icon={faTrashCan} /></li>
                                    }
