...
const ClientCards: React.FC = () => {
    ...
    return (
        <ul className='flex flex-col gap-3 w-full lg:w-1/2 xl:w-1/4 order-1 md:order-2 text-muted-foreground'>
            <h1 className='text-foreground border-b mb-1 pb-1 text-lg font-bold'>{t('profile.cards')}</h1>
            {cards && cards.length > 0 ? cards.map(card => (
                <div className='border bg-card shadow w-full h-44 flex flex-col gap-2 p-5' key={card.id}>
                    ...
                    <small className='h-5 w-56'>{t('info.date.buy')}: {new Date(card.created_at).toLocaleString()}</small>
                    <div className='w-full flex justify-between mt-auto h-8'>
                        <div className='h-7'>{t('balance.h1')}: <span className='text-primary font-bold'>{card.balance}</span></div>
                        <Button>{t('client.page.header.book_now')}</Button>
                    </div>
                </div>
            )) : cards && cards.length < 1 ? <div className='w-full px-3 py-2'>{t('global.no_data')}</div> :
                skeleton.map(item => (
