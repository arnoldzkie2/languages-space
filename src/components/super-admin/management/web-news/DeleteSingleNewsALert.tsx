...
const DeleteSingleNewsAlert = ({ news }: Props) => {

    const [open, setOpen] = useState(false)
    const { getNews } = useAdminNewsStore()
    const { setIsLoading, setErr } = useGlobalStore()

    const deleteSingleNews = async (e: React.FormEvent) => {
        try {
            e.preventDefault()
            setIsLoading(true)
            const { data } = await axios.delete('/api/news', { params: { newsID: news.id } })
...
export default DeleteSingleNewsAlert