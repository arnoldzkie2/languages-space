...
const AdminSupplierTable: React.FC<Props> = ({ filteredTable }) => {

    const { selectedSupplier, setSelectedSupplier, openViewSupplierModal, deleteSupplierWarning } = useAdminSupplierStore()
    const { openOperation, closeOperation, operation, selectedID, departmentID } = useGlobalStore()
    ...
    return (
        <table className="text-sm text-left text-gray-800 shadow-md w-full">
            ...
            <tbody>
                {filteredTable.length < 1 ? <Skeleton /> : filteredTable.map(supplier => (
                    <tr className="bg-white border hover:bg-slate-50" key={supplier.id}>
                        ...
                        <td className='py-3 relative px-6'>
                            ...
                            <ul className={`${operation && selectedID === supplier.id ? 'block' : 'hidden'} absolute bg-white p-3 gap-1 z-10 w-24 shadow-lg border flex flex-col text-gray-600`}>
                                ...
                                {permissions?.update_supplier && <Link href={`/admin/manage/supplier/update/${supplier.id}`} className='flex mb-1 justify-between items-center cursor-pointer hover:text-blue-600'>{tt('update')} <FontAwesomeIcon icon={faPenToSquare} /></Link>
                                }
                                {permissions?.delete_supplier && <li className='flex mb-1 justify-between items-center cursor-pointer hover:text-red-600' onClick={() => deleteSupplierWarning(supplier)}>{tt('delete')} <FontAwesomeIcon icon={faTrashCan} /></li>
                                }
...
export default AdminSupplierTable;
