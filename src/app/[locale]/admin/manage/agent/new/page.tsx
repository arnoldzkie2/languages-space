...
const Page = () => {
    ...
    const { isSideNavOpen, setIsLoading, setErr, prevProfileKey, setPrevProfileKey, deleteProfile, departmentID } = useGlobalStore()
    ...
    const createAgent = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setIsLoading(true)
            const { data } = await axios.post('/api/agent', { ...formData, departments: [departmentID] })
        ...
        <>
            ...
            <div className={`flex flex-col h-full pb-8 w-full gap-8 ${isSideNavOpen ? 'pl-40' : 'pl-16'}`}>
                ...
                <nav className={`border-b h-20 flex items-center bg-white px-8 justify-between`}>
                    <h1 className='font-black text-gray-600 text-xl uppercase'>{t('agent.create')}</h1>
                </nav>

                <div className='w-full px-8'>

                    <form className='w-1/2 flex flex-col gap-10 bg-white text-gray-600 p-10 border' onSubmit={createAgent}>
                        <Err />
                        ...
                        <div className='flex items-center gap-10 w-1/2 self-end'>
                            <Link href={'/admin/manage/agent'} className='flex items-center justify-center w-full h-10 rounded-md hover:bg-slate-200 border'>{tt('cancel')}</Link>
        ...
        <div className='w-full flex flex-col gap-4'>
            ...
            <div className='w-full flex flex-col gap-2'>
                ...
                <div className='flex items-center gap-5 w-full relative'>
                    ...
                    <div className={`absolute right-1 bg-white px-5 py-0.5 top-0.5 ${formData.commission_type === 'percentage' ? 'flex' : 'hidden'}`}>%</div>
