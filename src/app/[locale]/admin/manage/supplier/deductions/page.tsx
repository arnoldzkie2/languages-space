        ...
        <div>
            ...
            <div className={`flex flex-col h-full pb-8 w-full gap-8 ${isSideNavOpen ? 'pl-40' : 'pl-16'}`}>
                ...
                <nav className={`border-b h-20 flex items-center bg-white px-8 justify-between`}>
                    <h1 className='font-black text-gray-600 text-xl uppercase'>{t('supplier.deductions')}</h1>
                ...
                <div className='w-full px-8'>
                    <div className='flex p-10 w-1/2 border justify-between'>
                        ...
                        <div className='flex flex-col w-1/2 gap-5'>
                            ...
                            <ul className='flex flex-col gap-3 w-full max-h-[400px] overflow-y-auto text-gray-600'>
                                {supplier.length < 1 ? <Skeleton /> : supplier.map(supplier => (
                                    <li key={supplier.id}
                                        className={`${supplier.id === formData.supplierID && 'bg-blue-600 text-white'}
                                     border px-3 h-9 w-full py-1.5 cursor-pointer hover:bg-blue-600 hover:text-white rounded-md min-h-[2.5rem]`}
                        ...
                        <div className='flex flex-col w-1/3 gap-5'>
                            ...
                            <div className='flex w-full items-center gap-5'>
                                <Link href={'/admin/manage/supplier'} className='border py-2 w-1/2 flex items-center justify-center rounded-md hover:bg-slate-50'>{tt('cancel')}</Link>
...
const Skeleton = () => {
    ...
    return (
        <>
            {skeleton.map(item => (
                <li key={item} className='w-full bg-slate-100 rounded-md h-9 min-h-[2.5rem] animate-pulse'></li>
            ))}
        </>
    )
}
