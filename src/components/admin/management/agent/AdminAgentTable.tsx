...
const AdminAgentTable: React.FC<Props> = ({ filteredTable }) => {
                        ...
                        <tr className="bg-white border hover:bg-slate-50" key={agent.id}>
                            ...
                            <td className='py-3 relative px-6'>
                                ...
                                <ul className={`${operation && selectedID === agent.id ? 'block' : 'hidden'} absolute bg-white p-3 gap-1 z-10 w-24 shadow-lg border flex flex-col text-gray-600`}>
                                    ...
                                    {permissions?.update_agent && < Link href={`/admin/manage/agent/update/${agent.id}`} className='flex mb-1 justify-between items-center cursor-pointer hover:text-blue-600'>{tt('update')} <FontAwesomeIcon icon={faPenToSquare} /></Link>
                                    }
                                    {permissions?.delete_agent && <li className='flex mb-1 justify-between items-center cursor-pointer hover:text-red-600' onClick={() => openDeleteAgentModal(agent)}>{tt('delete')} <FontAwesomeIcon icon={faTrashCan} /></li>
                                    }
