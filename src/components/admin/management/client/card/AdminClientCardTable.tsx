...
const AdminClientCardTable: React.FC<Props> = ({ filteredTable, clientID }) => {
                        ...
                        <tr className="bg-white border hover:bg-slate-50" key={card.id}>
                            ...
                            <td className='py-3 relative px-6'>
                                ...
                                <ul className={`${operation && selectedID === card.id ? 'block' : 'hidden'} absolute bg-white p-3 gap-1 z-10 w-24 shadow-lg border flex flex-col text-gray-600`}>
                                    {permissions?.renew_cards && < button disabled={isLoading} onClick={(e) => renewClientCard({ e, clientCardID: card.id, clientID })} className={`flex mb-1 justify-between items-center cursor-pointer ${isLoading ? 'text-green-500' : 'hover:text-green-500'}`}>{tt('renew')} {isLoading ?
                                        <FontAwesomeIcon icon={faSpinner} width={16} height={16} className='animate-spin' /> : <FontAwesomeIcon icon={faRotateRight} width={16} height={16} />}
                                    </button>}
                                    ...
                                    {permissions?.update_client_cards && < Link href={`/admin/manage/client/card/${clientID}/update/${card.id}`} className='flex mb-1 justify-between items-center cursor-pointer hover:text-blue-600'>{tt('update')} <FontAwesomeIcon icon={faPenToSquare} /></Link>
                                    }
                                    {permissions?.delete_client_cards && <li className='flex mb-1 justify-between items-center cursor-pointer hover:text-red-600' onClick={() => openDeleteClientCardModal(card)}>{tt('delete')} <FontAwesomeIcon icon={faTrashCan} /></li>
                                    }
...
export default AdminClientCardTable;
