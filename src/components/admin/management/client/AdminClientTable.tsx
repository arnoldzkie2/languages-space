...
const AdminClientTable: React.FC<Props> = ({ filteredTable }) => {

    const { permissions } = useAdminPageStore()
                        ...
                        <tr className="bg-white border hover:bg-slate-50" key={client.id}>
                            ...
                            <td className='py-3 relative px-6'>
                                ...
                                <ul className={`${operation && selectedID === client.id ? 'block' : 'hidden'} absolute bg-white p-3 gap-1 z-10 w-24 shadow-lg border flex flex-col text-gray-600`}>
                                    {client.cards.length > 0 && permissions?.view_client_cards && <Link href={`/admin/manage/client/card/${client.id}`} className='flex mb-1 justify-between items-center cursor-pointer hover:text-orange-500'>{t('client.card.client')} <FontAwesomeIcon icon={faCreditCard} /></Link>}
                                    ...
                                    {permissions?.update_client && <Link href={`/admin/manage/client/update/${client.id}`} className='flex mb-1 justify-between items-center cursor-pointer hover:text-blue-600'>{tt('update')} <FontAwesomeIcon icon={faPenToSquare} /></Link>}
                                    {permissions?.delete_client && <li className='flex mb-1 justify-between items-center cursor-pointer hover:text-red-600' onClick={() => deleteWarning(client)}>{tt('delete')} <FontAwesomeIcon icon={faTrashCan} /></li>}
