...
export const getWeeklyOrderCountChart = async () => {
    ...
    const currentWeek = await prisma.order.findMany({
        where: {
            created_at: {
    ...
    const previousWeek = await prisma.order.findMany({
        where: {
            created_at: {
    ...
    // Initialize objects to hold order counts for each day of the week
    const currentWeekCounts: Record<string, number> = {};
    const previousWeekCounts: Record<string, number> = {};

    // Count the number of order for each day of the current week
    ...
    // Count the number of order for each day of the previous week
    previousWeek.forEach(order => {
        const day = format(order.created_at, 'eeee'); // Get the day name (e.g., Monday)
        previousWeekCounts[day] = (previousWeekCounts[day] || 0) + 1;
    });

    // Prepare data structure with total order counts for each day of the current and previous week
...
export const getMonthlyOrderCountChart = async () => {
    ...
    // Count the number of order for each day of the current month
    ...
    // Count the number of order for each day of the previous month
    const previousMonthCount: Record<number, number> = {};
    previousMonth.forEach(order => {
        const dayOfMonth = order.created_at.getDate();
        previousMonthCount[dayOfMonth] = (previousMonthCount[dayOfMonth] || 0) + 1;
    });

    // Prepare data structure with total order counts for each day of the current and previous month
...
export const getYearlyOrderCountChart = async () => {
    ...
    // Fetch order within the current year
    const currentYear = await prisma.order.findMany({
        where: {
            created_at: {
                gte: startDate,
                lte: endDate
    ...
    // Fetch order within the previous year
    const previousYear = await prisma.order.findMany({
        where: {
            created_at: {
    ...
    // Count the number of order for each month of the current year
    ...
    // Count the number of order for each month of the previous year
