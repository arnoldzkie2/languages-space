...
const AgentHeader = () => {
    ...
    return (
        <header className={`z-10 px-5 sm:px-10 md:px-16 lg:px-24 xl:px-36 2xl:px-44 h-16 md:h-20 fixed w-screen flex items-center top-0 left-0 justify-between text-gray-700 bg-white border-b`}>
            <Link href={'/agent/invite'} className='text-blue-600 font-black text-xl lg:text-2xl lg:w-96 tracking-tight'>LANGUAGES-SPACE</Link>
            ...
            <ul className={`w-full lg:flex lg:items-center lg:gap-5 ${isOpen ? 'gap-3 shadow-2xl flex flex-col fixed top-0 left-0 w-screen bg-white px-5 sm:px-10 md:px-16 md:pt-[8.7px] pb-5 pt-[8.2px] text-gray-600' : 'hidden'}`}>
                ...
                <div className='lg:ml-auto flex flex-col gap-3 lg:gap-4 lg:items-center lg:flex-row'>
                    <Link className='active:text-blue-600 w-28 lg:hover:text-blue-600 lg:text-center' href={'/agent/invite'}>{t("header.invite")}</Link>
                </div>
                <div className={`mt-2 lg:mt-0 flex items-center gap-5 lg:ml-auto`}>
                    <Link href={'/agent/profile'} className='flex items-center gap-2 px-3 py-1 border hover:bg-slate-100 rounded-md'>
