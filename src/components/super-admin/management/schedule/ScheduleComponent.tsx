...
const ScheduleComponent: React.FC<Props> = (schedule) => {
    ...
    return (
        <div onClick={() => status === 'reserved' ? viewBooking(bookingID || '') :
            bindSchedule(scheduleData)} className={`p-2 mx-2 my-0.5 w-full border hover:border-blue-600 ${status === 'reserved' ?
                'bg-blue-600 text-white' : status === 'available' ? 'bg-slate-200 text-gray-600' :
                    'bg-orange-500'} cursor-pointer rounded-md flex items-center gap-2`}>
