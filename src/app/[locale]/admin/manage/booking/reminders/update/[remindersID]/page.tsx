...
const Page = ({ params }: Props) => {
    ...
    const updateReminders = async (e: React.FormEvent) => {

        e.preventDefault()

        try {

            const { clientCardID, clientID, meeting_info, supplierID, scheduleID, client_quantity, supplier_quantity, note, courseID, name, settlement } = formData
    ...
    const confirmRemindersBooking = async (e: React.MouseEvent) => {
        ...
        <div className='h-screen'>
            ...
            <div className={`flex flex-col h-full pb-8 w-full gap-8 ${isSideNavOpen ? 'pl-40' : 'pl-16'}`}>
                ...
                <nav className={`border-b px-8 flex items-center min-h-[64px] justify-between`}>
                    <h1 className='font-black text-xl uppercase'>{t('booking.reminders.update')}</h1>
                ...
                <div className='w-full px-8 h-full'>
                    <Card className='w-1/3'>
                        <CardHeader>
                            <CardTitle>{t("booking.reminders.update")}</CardTitle>
                            <CardDescription><Err /></CardDescription>
                        </CardHeader>
                        <CardContent>
                            {formData.status ? <form onSubmit={updateReminders} className='w-full'>
                                ...
                                <div className='flex w-full h-full gap-10'>
                                    <div className='flex flex-col w-full gap-4'>
                                        ...
                                        <div className="w-full items-center gap-1.5">
                                            ...
                                            <Select onValueChange={(scheduleID) => setFormData(prev => ({ ...prev, scheduleID }))} value={formData.scheduleID}>
                                                ...
                                                <SelectContent>
                                                    <SelectGroup>
                                                        <SelectLabel>{formData.supplierID && supplierSchedule.length === 0 ? t('schedule.no-schedule') : !formData.supplierID ? t('supplier.select-first') : tt('supplier')}</SelectLabel>
                                        ...
                                        <SubmitButton type='button' msg={t('booking.confirm')} onClick={confirmRemindersBooking} />
                                    ...
                                    <div className='flex flex-col w-full gap-4'>
                                        ...
                                        <SubmitButton style='mt-auto w-full' msg={tt('update')} />
                                    </div>
                                </div>
                            </form> : <FontAwesomeIcon icon={faSpinner} className='animate-spin' width={30} height={30} />}
                        </CardContent>
                    </Card>
