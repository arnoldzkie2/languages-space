...
const NewScheduleModal = () => {
    ...
    const { isLoading, setIsLoading, skeleton, setErr } = useAdminGlobalStore()
    ...
    const addSchedule = async (e: any) => {
        ...
        e.preventDefault()
        if (!bookingFormData.supplierID) return setErr('Select a supplier')
        ...
        try {
            ...
            const { data } = await axios.post(`/api/schedule`, {
                supplierID: bookingFormData.supplierID,
