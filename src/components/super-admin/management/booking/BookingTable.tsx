...
const BookingTable: React.FC<Props> = ({ filteredTable }) => {
    ...
    const { openDeleteWarningModal, closeDeleteWarningModal, getBookings, selectedBookings, setSelectedBookings } = useAdminBookingStore()
                        ...
                        <tr className="bg-white border hover:bg-slate-50" key={booking.id}>
                            ...
                            <td className='py-3 relative px-5'>
                                ...
                                <ul className={`${operation && selectedID === booking.id ? 'block' : 'hidden'} absolute bg-white p-3 gap-1 z-10 w-24 shadow-lg border flex flex-col text-gray-600`}>
                                    ...
                                    <li className='flex mb-1 justify-between items-center cursor-pointer hover:text-red-600' onClick={() => openDeleteWarningModal(booking)}>{tt('delete')} <FontAwesomeIcon icon={faTrashCan} /></li>
