...
const AdminOrderTable: React.FC<Props> = ({ filteredTable }) => {
                        ...
                        <tr className="bg-white border hover:bg-slate-50" key={order.id}>
                            ...
                            <td className='py-3 relative px-4'>
                                ...
                                <ul className={`${operation && selectedID === order.id ? 'block' : 'hidden'} absolute bg-white p-3 gap-1 z-10 w-24 shadow-lg border flex flex-col text-gray-600`}>
                                    ...
                                    {permissions?.update_orders && <Link href={`/admin/manage/orders/update/${order.id}`} className='flex mb-1 justify-between items-center cursor-pointer hover:text-blue-600'>{tt('update')} <FontAwesomeIcon icon={faPenToSquare} /></Link>
                                    }
                                    {permissions?.delete_orders && <li className='flex mb-1 justify-between items-center cursor-pointer hover:text-red-600'>{tt('delete')} <FontAwesomeIcon icon={faTrashCan} /></li>
                                    }
