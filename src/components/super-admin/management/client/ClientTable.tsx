...
const ClientTable: React.FC<Props> = ({ clients, deleteWarning, clientSelectedID, operation, handleOperation, closeOperation }) => {
                ...
                {clients && clients.length > 0 ?
                    clients.map(client => (
                        <tr className="bg-white border hover:bg-slate-50" key={client.id}>
                            ...
                            <td className='px-6 py-4'>{client.phone_number ? client.phone_number : 'No Data'}</td>
                            <td className="px-6 py-4">{client.organization ? client.organization : 'No Data'}</td>
                            <td className="px-6 py-4">{client.origin ? client.origin : 'No Data'}</td>
