...
interface Props {
    ...
    filteredTable: AgentEarnings[]
    agentID: string
}

const AgentEarningsTable: React.FC<Props> = ({ filteredTable, agentID }) => {

    const { selectedEarnings, setSelectedEarnings } = useAgentEarningsStore()
    ...
    const handleSelection = (agent: AgentEarnings) => {
    ...
    const selectAllRows = () => {
        ...
        let updatedSelectedClients: AgentEarnings[]
    ...
    return (
        <div className='flex w-full flex-col'>
                            ...
                            <tr className="bg-card border hover:bg-muted hover:text-foreground" key={earnings.id}>
                                ...
                                <td className='py-3 relative px-6'>
                                    ...
                                    <ul className={`${operation && selectedID === earnings.id ? 'block' : 'hidden'} absolute bg-card p-3 gap-1 z-10 w-32 shadow-lg border flex flex-col text-muted-foreground`}>
                                        {isAdminAllowed('delete_agent_earnings') && <DeleteEarningsAlert earnings={earnings}
                                            agentID={agentID}
                                        />}
            ...
            <DeleteSelectedEarningsAlert agentID={agentID} />
        </div>
    );
};

export default AgentEarningsTable;
