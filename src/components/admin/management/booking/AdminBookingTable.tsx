...
const AdminBookingTable: React.FC<Props> = ({ filteredTable }) => {
                        ...
                        <tr className="bg-white border hover:bg-slate-50" key={booking.id}>
                            ...
                            <td className='py-3 relative px-2'>
                                ...
                                <ul className={`${operation && selectedID === booking.id ? 'block' : 'hidden'} absolute bg-white p-3 gap-1 z-10 w-24 shadow-lg border flex flex-col text-gray-600`}>
                                    {permissions?.update_booking && <Link href={`/admin/manage/booking/update/${booking.id}`} className='flex mb-1 justify-between items-center cursor-pointer hover:text-blue-600'>{tt('update')} <FontAwesomeIcon icon={faPenToSquare} /></Link>
                                    }
                                    {booking.status !== 'canceled' && permissions?.cancel_booking && <button disabled={isLoading} className='flex mb-1 justify-between items-center cursor-dpointer hover:text-red-600' onClick={(e: any) => cancelBooking(e, booking.id)}>
                                    ...
                                    {permissions?.delete_booking && <li className='flex mb-1 justify-between items-center cursor-pointer hover:text-red-600' onClick={() => openDeleteBookingWarningMOdal(booking)}>{tt('delete')} <FontAwesomeIcon icon={faTrashCan} /></li>
                                    }
