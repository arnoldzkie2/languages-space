...
const AdminNewsTable: React.FC<NewsTableProps> = ({ filteredTable }) => {
                        ...
                        <tr className="bg-white border hover:bg-slate-50" key={news.id}>
                            ...
                            <td className="py-3 px-6 h-5 w-14 relative">
                                ...
                                <ul className={`${operation && selectedID === news.id ? 'block' : 'hidden'} absolute bg-white p-3 gap-1 z-10 w-24 shadow-lg border flex flex-col text-gray-600`}>
                                    ...
                                    {permissions?.update_news && <Link href={`/admin/manage/news/update/${news.id}`} className='flex mb-1 justify-between items-center cursor-pointer hover:text-blue-600'>{tt('update')} <FontAwesomeIcon icon={faPenToSquare} /></Link>
                                    }
                                    {permissions?.delete_news && <li className='flex mb-1 justify-between items-center cursor-pointer hover:text-red-600' onClick={() => openNewsDeleteWarning(news)}>{tt('delete')} <FontAwesomeIcon icon={faTrashCan} /></li>
                                    }
