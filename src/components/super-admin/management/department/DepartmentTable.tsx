                        ...
                        <tr className="bg-card border hover:bg-muted hover:text-foreground" key={department.id}>
                            ...
                            <td className='py-3 relative px-6'>
                                ...
                                <ul className={`${operation && selectedID === department.id ? 'block' : 'hidden'} absolute bg-card p-3 gap-1 z-10 w-24 shadow-lg border flex flex-col text-muted-foreground`}>
                                    <li onClick={() => openUpdateDepartment(department)} className='flex mb-1 justify-between items-center cursor-pointer hover:text-foreground'>{tt('update')} <FontAwesomeIcon icon={faPenToSquare} /></li>
                                    <li className='flex mb-1 justify-between items-center cursor-pointer hover:text-foreground' onClick={() => openDeleteDepartment(department)}>{tt('delete')} <FontAwesomeIcon icon={faTrashCan} /></li>
