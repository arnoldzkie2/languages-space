...
const CreateSupplierCommentAlert = ({ booking }: { booking: BookingProps }) => {
    ...
    const createSupplierComment = async (e: React.FormEvent) => {
        ...
        try {
            setIsLoading(true)
            const { data } = await axios.post('/api/booking/comments/supplier', {
                rate,
                message,
                bookingID: booking.id,
                homework,
            ...
            if (data.ok) {
                getBookings()
                setIsLoading(false)
                toast.success("Success! comments created")
                setOpen(false)
            }
    ...
    return (
        <AlertDialog open={open} onOpenChange={setOpen}>
            ...
            <AlertDialogContent>
                ...
                <form className='w-full flex flex-col gap-5 text-muted-foreground overflow-y-auto px-2 max-h-[600px]' onSubmit={createSupplierComment}>
...
export default CreateSupplierCommentAlert